...
下面这张图把当前实现中的正常执行、用户提问、Web 断连取消、重连和进程重启放在同一张图中。实线表示正常执行路径，虚线表示取消、持久化或恢复相关路径。
...
flowchart TD
    ...
    subgraph DISCONNECT["Web 断连与用户取消"]
        SSE["最后一个 SSE 连接断开"] --> GRACE["GracePeriod<br/>默认 20 秒"]
        GRACE --> RECONNECT{"宽限期内重连?"}
        RECONNECT -->|"是"| KEEP["撤销 timer<br/>原 Run 继续"]
        RECONNECT -->|"否"| DISCONNECTED["OnDisconnect(session_id)"]
        DISCONNECTED --> CANCEL_SESSION["Loop.CancelSession"]
        CANCEL_SESSION --> IS_PENDING{"是否 pending question?"}
        IS_PENDING -->|"是"| EXEMPT["豁免取消<br/>继续等待回答"]
        IS_PENDING -->|"否"| CANCEL_CTX["调用 run cancel()"]
    ...
    class SSE,GRACE,DISCONNECTED,CANCEL_SESSION,CANCEL_CTX,CTX_DONE,EXEMPT cancel;
...
Model 状态主要写入 Trace，用于记录请求耗时、首 token 延迟、流式过程和 Provider 错误，不写入 Conversation。
...
模型和工具状态通过 `StreamSink` 回调给 Loop，Loop 将它们写入 Trace；正文、推理、tool call 和 tool result 继续通过 Outbound Bus 推给 Channel。
...
Web 使用 SSE 接收输出。刷新、切后台、网络切换和代理重连都可能造成 SSE 断开，因此 WebChannel 先启动宽限期，默认 20 秒：

```text
最后一个 SSE 订阅者断开
  -> 启动 GracePeriod
  -> 同 session 重连：撤销 timer，Run 继续
  -> 宽限期结束且仍无连接：OnDisconnect(session_id)
  -> Loop.CancelSession(session_id)
```

同一 session 有多个 tab 时，只要还有一个订阅者在线，就不会进入取消流程。

### `CancelSession` 的实际行为

`Loop.CancelSession(sessionID)` 的逻辑是：

1. 检查该 session 是否正在等待 `ask_user_question` 的回答。
2. 如果是等待态，直接豁免，不取消。
3. 否则取出 `running` registry 中的 cancel function。
4. 调用 `cancel()`，让取消沿 context 向下传播。
...
当前 yomi 没有独立的“取消当前 Run”用户命令或 API。Web 场景中的用户取消实际表现为：用户离开页面，SSE 最后一个连接断开，宽限期结束后触发 `CancelSession`。CLI 的 `Ctrl+C` 取消的是进程级 context，会影响所有正在运行的 session，而不是单个 Run。
...
SSE 断开时，`CancelSession` 因为发现 pending question 而豁免取消。进程退出或 Run 自身超时仍会结束等待。
...
这不是“取消后恢复”，而是“取消尚未发生”。Run ID、context 和 Snapshot 都保持不变。
