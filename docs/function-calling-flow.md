...
本文档归档 `szabot` 当前 Function Calling 的完整链路：应用启动时如何显式注册工具，用户消息如何让模型决定调用工具，工具结果又如何回到模型，最终形成回复。

当前实现是非流式的 OpenAI Chat Completions 兼容方案；`OpenAICompatibleProvider` 可用于 DeepSeek、OpenAI、Moonshot、Ollama 等兼容 `/chat/completions` 的服务。
...
当前工具注册在 `cmd/szabot/main.go` 的装配阶段完成。程序会将启动时的当前工作目录作为工作区，创建一个空的 `tools.Registry`，然后创建并注册 `read_file`。
...
| 工具名 | 用途 | 作用范围 |
|---|---|---|
| `read_file` | 读取 UTF-8 文本文件 | 启动目录对应的 workspace 内 |
...
sequenceDiagram
    ...
    Loop->>Loop: 构造 [{role: user, content: ...}]
    Loop->>Runner: Run(ctx, messages)
    ...
    Runner-->>Loop: 最终回复文本
    Loop->>Bus: PublishOutbound(OutboundMessage)
...
当前实现已经支持本地 `read_file` 的完整 Function Calling 闭环，但仍有明确边界：

- 只有一个只读工具；
- 只支持非流式请求；
- 没有 session 持久化，用户下一条独立消息不会自动带上前一条工具调用历史；
- 同一轮调用串行执行；
- 未接入 MCP。
