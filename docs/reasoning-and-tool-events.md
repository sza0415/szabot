...
- [provider.go](/Users/ziangsun/Documents/szabot/internal/providers/provider.go) — `Message` / `ChatResponse` / `StreamChunk` 补 reasoning 字段
- [openai_compatible.go](/Users/ziangsun/Documents/szabot/internal/providers/openai_compatible.go) — 非流式解析 `reasoning_content`
- [stream.go](/Users/ziangsun/Documents/szabot/internal/providers/stream.go) — 流式解析推理增量
- [runner.go](/Users/ziangsun/Documents/szabot/internal/agent/runner.go) — 新增 `StreamSink` / `RunResult` / `RunCollect`，收集完整消息序列
- [events.go](/Users/ziangsun/Documents/szabot/internal/bus/events.go) — `OutboundMessage` 增加 `Kind`（事件类型）
- [loop.go](/Users/ziangsun/Documents/szabot/internal/agent/loop.go) — 分类推送出站 + 落盘完整轨迹
- [cli.go](/Users/ziangsun/Documents/szabot/internal/channels/cli.go) — `writeLoop` 按 Kind 分区渲染
- [web.go](/Users/ziangsun/Documents/szabot/internal/channels/web.go) — SSE 载荷带上 `kind`
...
[events.go](/Users/ziangsun/Documents/szabot/internal/bus/events.go) 给出站消息引入 `Kind`：
...
[loop.go](/Users/ziangsun/Documents/szabot/internal/agent/loop.go) 的 `handle`：
...
- **CLI**（[cli.go](/Users/ziangsun/Documents/szabot/internal/channels/cli.go)）：`writeLoop` 按 `Kind` 分区——推理以暗色 `[思考]` 段显示、工具调用/结果各占一行带 `⚙` 标记、正文沿用 `szabot>` 前缀。用局部标志跟踪推理段的开合（单 goroutine，无需锁）。
- **Web**（[web.go](/Users/ziangsun/Documents/szabot/internal/channels/web.go)）：SSE 载荷新增 `kind` 字段（空时归一为 `"answer"`），前端可据此把思考/工具/正文分区展示。
...
| 测试文件 | 新增覆盖点 |
|---|---|
| [openai_compatible_test.go](/Users/ziangsun/Documents/szabot/internal/providers/openai_compatible_test.go) | `TestChatParsesReasoningContent`：非流式 `reasoning_content` 解析进 `Reasoning` 且不污染 `Content` |
| [stream_test.go](/Users/ziangsun/Documents/szabot/internal/providers/stream_test.go) | `TestChatStreamReasoning`：流式推理增量单独回调为 `ReasoningDelta`、与正文互不混淆 |
| [runner_test.go](/Users/ziangsun/Documents/szabot/internal/agent/runner_test.go) | `TestRunCollectCapturesReasoningAndToolMessages`：`RunResult.Messages` 含 assistant(推理+tool_calls) → tool 结果 → 最终 assistant，且各类回调触发正确 |
| [session_test.go](/Users/ziangsun/Documents/szabot/internal/agent/session_test.go) | `TestLoopPersistsReasoningAndToolCalls`：一轮工具调用后，重新读盘验证 user → assistant(推理+tool_calls) → tool 结果 → 最终 assistant 完整落盘 |
