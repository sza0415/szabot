...
- [provider.go](/Users/ziangsun/Documents/szabot/internal/providers/provider.go) — 消息结构补 JSON tag、新增流式接口
- [stream.go](/Users/ziangsun/Documents/szabot/internal/providers/stream.go) — OpenAI 兼容 provider 的 SSE 实现（新增）
- [session.go](/Users/ziangsun/Documents/szabot/internal/agent/session.go) — jsonl SessionStore（新增）
- [loop.go](/Users/ziangsun/Documents/szabot/internal/agent/loop.go) — 加载/回写历史 + 改用流式
- [runner.go](/Users/ziangsun/Documents/szabot/internal/agent/runner.go) — 新增 `RunStream`，探测并透传流式
- [events.go](/Users/ziangsun/Documents/szabot/internal/bus/events.go) — `OutboundMessage` 增加 `Delta` / `Done` 标记
- [cli.go](/Users/ziangsun/Documents/szabot/internal/channels/cli.go) — `writeLoop` 按标记增量输出
- [main.go](/Users/ziangsun/Documents/szabot/cmd/szabot/main.go) — 装配 SessionStore
...
新增 [session.go](/Users/ziangsun/Documents/szabot/internal/agent/session.go) 中的 `SessionStore`：
...
[provider.go](/Users/ziangsun/Documents/szabot/internal/providers/provider.go) 新增：
...
新增 [stream.go](/Users/ziangsun/Documents/szabot/internal/providers/stream.go)，`OpenAICompatibleProvider.ChatStream`：
...
[events.go](/Users/ziangsun/Documents/szabot/internal/bus/events.go) 给 `OutboundMessage` 增加：
...
[cli.go](/Users/ziangsun/Documents/szabot/internal/channels/cli.go) 的 `writeLoop` 按标记打印：
...
sequenceDiagram
    ...
    participant Loop as Loop.handle
    ...
    User->>Loop: 输入一行
    Loop->>Runner: RunStream(msgs, onDelta)
    ...
    loop 每个 SSE data 块
        ...
        Runner->>Loop: onDelta(段)
        Loop->>Bus: OutboundMessage{Delta:true, Text:段}
        Bus-->>Write: 收到 Delta
    ...
    Runner-->>Loop: reply（完整）
    Loop->>Loop: Store.Append(user, assistant)
    Loop->>Bus: OutboundMessage{Done:true}
    Bus-->>Write: 收到 Done
    Write->>User: 换行 + 重打 "> "
```
...
| 测试文件 | 覆盖点 |
|---|---|
| [session_test.go](/Users/ziangsun/Documents/szabot/internal/agent/session_test.go) | jsonl 落盘 round-trip、跨 store 实例重读、session 隔离；**核心回归**：同一 session 第二次请求带上 `system + user(first) + assistant(first-reply) + user(second)`；不同 SessionID 互不串扰 |
| [stream_test.go](/Users/ziangsun/Documents/szabot/internal/providers/stream_test.go) | SSE 正文增量顺序与累积、tool_calls 跨片拼装（还原成 `{"pattern":"foo"}`）、HTTP 401 错误 |
| [runner_test.go](/Users/ziangsun/Documents/szabot/internal/agent/runner_test.go) | `RunStream` 流式增量逐段透传、provider 不支持流式时回退为整段回调 |
| [cli_test.go](/Users/ziangsun/Documents/szabot/internal/channels/cli_test.go) | 多条 Delta 拼成同一前缀下的整段、前缀只出现一次、Done 后补提示符、忽略其他 channel 的消息 |
