...
func TestRunAPIListsSnapshotsAndFiltersStatus(t *testing.T) {
	...
	if err := cancelled.Transition(agent.RunCancelled, "client disconnected"); err != nil {
		t.Fatal(err)
	}
	...
	if detailRec.Code != http.StatusOK || !strings.Contains(detailRec.Body.String(), "client disconnected") {
...
// TestDisconnectCancelsAfterGrace 验证：最后一个订阅者断开后，等过宽限期
// 仍无重连，OnDisconnect 会被调用（携带正确的 sessionID）。
func TestDisconnectCancelsAfterGrace(t *testing.T) {
	b := bus.New(16)
	w := newTestWeb(b)
	w.GracePeriod = 30 * time.Millisecond

	got := make(chan string, 1)
	w.OnDisconnect = func(sessionID string) { got <- sessionID }

	sub := &subscriber{sessionID: "S", events: make(chan bus.OutboundMessage, 4)}
	w.addSubscriber(sub)
	w.removeSubscriber(sub) // 最后一个断开 → 启动宽限 timer

	select {
	case s := <-got:
		if s != "S" {
			t.Fatalf("OnDisconnect got %q, want S", s)
		}
	case <-time.After(time.Second):
		t.Fatal("OnDisconnect should fire after grace period")
	}
}

// TestDisconnectNotCancelledImmediately 验证：断开后在宽限期内不会立即取消
// （给重连留窗口）。
func TestDisconnectNotCancelledImmediately(t *testing.T) {
	b := bus.New(16)
	w := newTestWeb(b)
	w.GracePeriod = 200 * time.Millisecond
	...
	select {
	case s := <-got:
		t.Fatalf("should NOT cancel within grace period, got %q", s)
	case <-time.After(50 * time.Millisecond):
		// 正确：宽限期内还没触发。
	}
}

// TestReconnectCancelsGraceTimer 验证：断开后、宽限期到点前，同 session 重连
// 会撤销宽限 timer，最终不触发取消。这模拟锁屏/网络抖动后 EventSource 自动重连。
func TestReconnectCancelsGraceTimer(t *testing.T) {
	b := bus.New(16)
	w := newTestWeb(b)
	w.GracePeriod = 100 * time.Millisecond

	got := make(chan string, 1)
	w.OnDisconnect = func(sessionID string) { got <- sessionID }

	sub1 := &subscriber{sessionID: "S", events: make(chan bus.OutboundMessage, 4)}
	w.addSubscriber(sub1)
	w.removeSubscriber(sub1) // 断开 → 启动宽限 timer

	// 宽限期内重连（同 session 新连接接入）。
	time.Sleep(30 * time.Millisecond)
	sub2 := &subscriber{sessionID: "S", events: make(chan bus.OutboundMessage, 4)}
	w.addSubscriber(sub2) // 应撤销 timer

	select {
	case s := <-got:
		t.Fatalf("reconnect should have cancelled the timer, but OnDisconnect fired with %q", s)
	case <-time.After(200 * time.Millisecond):
		// 正确：重连撤销了 timer，未触发取消。
	}
}

// TestMultiTabNoCancelWhileOthersAlive 验证：一个 session 开多标签页时，
// 断开其中一个连接不触发取消（还有活订阅者）。
func TestMultiTabNoCancelWhileOthersAlive(t *testing.T) {
	b := bus.New(16)
	w := newTestWeb(b)
	w.GracePeriod = 30 * time.Millisecond
	...
	select {
	case s := <-got:
		t.Fatalf("should NOT cancel while another tab is alive, got %q", s)
	case <-time.After(100 * time.Millisecond):
		// 正确：还有 tab2，不启动宽限 timer、不取消。
	}
}
