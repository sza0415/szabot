...
// TestCancelSessionExemptsPending 验证：当 session 正处于 ask_user_question
// 挂起态（在等用户回答）时，CancelSession 豁免取消，任务不被 kill。
func TestCancelSessionExemptsPending(t *testing.T) {
	loop := &Loop{}
	...
	if cancelled {
		t.Fatal("CancelSession should be a no-op while session is pending an answer")
	}
}
