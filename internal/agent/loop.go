...
func (l *Loop) enqueue(ctx context.Context, in bus.InboundMessage) {
	...
	l.record(ctx, run, tracing.EventRunQueued, string(RunQueued), nil)
...
// CancelSession 取消某个 session 正在运行的任务（中断下游 Runner/LLM）。
//
// 由 channel 侧在"客户端断连且过宽限期仍无重连"时调用。
//
// 豁免规则：若该 session 正处于 ask_user_question 挂起态（在等用户回答），
// 则不取消——此时"用户人不在页面"是正常的（在思考/查资料），kill 掉体验很差。
// 找不到运行中的任务（已结束/从未开始）时静默返回。
