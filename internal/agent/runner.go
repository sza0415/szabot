	...
	for turn := 0; turn < maxTurns; turn++ {
		...
		for _, call := range response.ToolCalls {
			...
			if sink.OnToolStatus != nil {
				sink.OnToolStatus(ToolExecutionEvent{Call: call, Status: ToolPending})
			}
			...
			if sink.OnToolStarted != nil {
				sink.OnToolStarted(ToolExecutionEvent{Call: call, Status: toolStatus})
			}
			if sink.OnToolStatus != nil {
				sink.OnToolStatus(ToolExecutionEvent{Call: call, Status: toolStatus})
			}
			...
			if sink.OnToolFinished != nil {
				sink.OnToolFinished(ToolExecutionEvent{Call: call, Status: toolStatus, Result: result, Duration: duration, Err: toolErr})
			}
			if sink.OnToolStatus != nil {
				sink.OnToolStatus(ToolExecutionEvent{Call: call, Status: toolStatus, Result: result, Duration: duration, Err: toolErr})
			}
