...
- Provider、工具、持久化和客户端断连等故障可以主动注入。
- 一次运行可以同时检查用户输出、工具轨迹、Trace、Conversation 和资源消耗。

| 方向 | 解决的问题 | 代表能力 |
| --- | --- | --- |
| 运行时 Harness | Agent 执行时如何感知状态、交互和停止 | 状态栏、用户提问、断连取消、流式推送 |
...
- 断连取消：客户端断开后取消下游任务，并处理宽限期和 pending question。
